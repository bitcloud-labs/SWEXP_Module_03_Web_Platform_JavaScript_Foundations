#!/usr/bin/env node
/**
 * Forge SWEXP autograder (Module 03 — JavaScript Foundations).
 * Runs every exercise's behaviour tests (vitest, Node + jsdom), then prints a
 * per-exercise score and writes a Markdown report for GitHub Actions.
 *
...
// Exercise folder name from a test file path, e.g. ".../labs/lab-04-js-runtime/tests/x.test.js"
function exerciseOf(p) {
  const m = p.replace(/\\/g, '/').match(/\/(labs|assignments)\/([^/]+)\//);
  return m ? `${m[1]}/${m[2]}` : null;
}

// 1) Behaviour tests (Node + jsdom).
...
// 2) No separate type gate in a JavaScript module — the tests are the gate.
const typeGate = { ok: true };
...
const md = [
  ...
  ``,
  complete
    ? `🎉 **All exercises complete — every behaviour test passes.**`
